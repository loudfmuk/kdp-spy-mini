        ...
        try:
            ...
            for item in items:
                try:
                    ...
                    # === IMPROVED BSR EXTRACTION ===
                    bsr = "N/A"
                    text = item.get_text()
                    
                    # Multiple patterns Amazon uses
                    bsr_match = (re.search(r'#([\d,]+)\s+in\s+Books', text) or 
                                re.search(r'Best Sellers Rank.*?#([\d,]+)', text) or
                                re.search(r'#([\d,]{3,})', text))
                    
                    if bsr_match:
                        bsr = bsr_match.group(1)
...
with tab2:
    st.info("ISBN Lookup coming soon")

st.caption("💡 Tip: Try broader keywords like 'coloring book' or 'adult coloring'")
